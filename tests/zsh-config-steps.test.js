...
describe("zsh-config step", () => {
  ...
  test("writeZshrc previews and merges suitup-managed .zshrc additions without removing local edits", async () => {
  ...
  test("writeZshenv creates .zshenv in empty sandbox", async () => {
    await writeZshenv({ home: sandbox.path });

    expect(existsSync(join(sandbox.path, ".zshenv"))).toBe(true);
    const content = readFileSync(join(sandbox.path, ".zshenv"), "utf-8");
    expect(content).toContain("Generated by suitup");
    expect(content).toContain("paths.zsh");
    expect(content).toContain("env.zsh");
  });

  test("writeZshenv previews and merges suitup-managed .zshenv additions without removing local edits", async () => {
    writeFileSync(join(sandbox.path, ".zshenv"), "# Generated by suitup\nold content", "utf-8");
    mockConfirm.mockResolvedValue(true);
  ...
  test("writeZshenv skips non-suitup .zshenv", async () => {
    writeFileSync(join(sandbox.path, ".zshenv"), "# my custom zshenv\nexport FOO=bar", "utf-8");
  ...
  test("backupShellRcFiles includes .zshenv in backups", async () => {
    writeFileSync(join(sandbox.path, ".zshrc"), "zshrc content\n", "utf-8");
    writeFileSync(join(sandbox.path, ".zshenv"), "zshenv content\n", "utf-8");

    const result = await backupShellRcFiles({ home: sandbox.path, reason: "setup" });

    expect(result.files).toContain(".zshenv");
    expect(existsSync(join(result.backupDir, ".zshenv"))).toBe(true);
  });
...
describe("aliases step", () => {
  ...
  test("merges suitup-managed alias additions while preserving user aliases", async () => {
    mkdirSync(join(sandbox.path, ".config", "zsh", "shared"), { recursive: true });
    writeFileSync(
    ...
    const content = readFileSync(join(sandbox.path, ".config", "zsh", "shared", "aliases.zsh"), "utf-8");
    expect(content).toContain("alias edit-zsh=");
