...
import { detectCompletedSteps, getDefaultSteps, getInitialStepValues, isZshShell } from "../src/setup.js";
...
describe("Setup simulation in sandbox", () => {
  ...
  test("detects completed suitup-managed setup steps", () => {
    ...
    mkdirSync(join(sandbox, ".config", "suitup"), { recursive: true });
    ...
    writeFileSync(join(sandbox, ".config", "suitup", "zinit-plugins"), "", "utf-8");
    writeFileSync(join(sandbox, ".config", "suitup", "aliases"), "", "utf-8");
  ...
  test("removes completed steps from initial setup selections", () => {
    ...
    try {
      ...
      mkdirSync(join(completedSandbox.path, ".config", "suitup"), { recursive: true });
      ...
      writeFileSync(join(completedSandbox.path, ".config", "suitup", "aliases"), "", "utf-8");
