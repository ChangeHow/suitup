...
describe("Setup simulation in sandbox", () => {
  ...
  test("can create full config structure in sandbox", () => {
    ...
    // Copy shared configs
    for (const file of ["tools.zsh", "fzf.zsh", "prompt.zsh", "completion.zsh", "highlighting.zsh", "aliases.zsh", "plugins.zsh"]) {
      copyFileSync(
  ...
  test("detects completed suitup-managed setup steps", () => {
    ...
    writeFileSync(join(sandbox, ".config", "zsh", "shared", "fzf.zsh"), "", "utf-8");
  ...
  test("removes completed steps from initial setup selections", () => {
    ...
    try {
      ...
      writeFileSync(join(completedSandbox.path, ".config", "zsh", "shared", "fzf.zsh"), "", "utf-8");
